...
import CourseSpecificStyleSettingExtendAction from "redux/ResultOutput/ResultsTblCollectOutput/CourseSpecificStyleSettingExtend.action";
import WS0061015_CheckYesNoNo from "pages/IN_InputBusiness/V4DS0212000_ProgressSetting/WS0061015_CheckYesNoNo";
import WS0457004_StyleSetting from "./WS0457004_StyleSetting";
import  ModalDraggable  from "components/Commons/ModalDraggable";

class WS0457001_CourseSpecificStyleSettingExtend extends React.Component {
  ...
  getScreenData(reload) {
    ...
    CourseSpecificStyleSettingExtendAction.getListData()
      .then((res) => {
        let data = res ? res : [];
  ...
  updateRecordData(index) {
    ...
    if (this.checkDuplicateCode()) {
      ...
      CourseSpecificStyleSettingExtendAction.saveData(params)
        .then((res) => {
          ...
          <WS0061015_CheckYesNoNo
            ...
            onFinishScreen={(ouput) => {
              if (ouput.Lio_StsReturn) {
                CourseSpecificStyleSettingExtendAction.daleteData(params)
                  .then(res => {
              ...
              onClick={() => {
                this.setState({
                  childModal: {
                    ...
                    width: 400,
                    component:
                      <WS0457010_PattanCopy
                        Li_ItemId={this.state.rowSelected.length > 0 ? this.state.rowSelected[0].PatternClassify : null}
...
export default connect(mapStateToProps, mapDispatchToProps)(WS0457001_CourseSpecificStyleSettingExtend);
