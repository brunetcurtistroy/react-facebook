...
class WS1039001_InspectNameConvertInMaintainTa extends React.Component {
  ...
  checkAddItem() {
    if (this.state.dataSource.length > 0) {
      let index = -1
      let index2 = -1
      index = this.state.dataSource.findIndex(x => !x.exam_name_short_name);
      index2 = this.state.dataSource.findIndex(x => !x.exam_code);
  ...
  checkDuplicateCode() {
    ...
    const uniqueValues = new Set(lstData.map(v => v.exam_name_short_name));
  ...
  handleDeleteRowTable(index) {
    ...
    if (dataCopy.length > 0) {
      ...
      this.setState({
        dataSource: dataCopy,
        selectedRowIndex: 0,
        selectedRowKeys: [dataCopy[0].id],
        selectedRowKey: dataCopy[0]
      })
      this.formRef.current?.setFieldsValue({
        dataTable: dataCopy,
      });
  ...
  getTableScroll(extraHeight = undefined, id = null) {
            ...
            >
              <Table.Column
                ...
                render={(value, record, index) => {
                  return (
                    <Form.Item name={["dataTable", index, 'exam_name_short_name']} style={{ marginBottom: "0" }}
                      rules={[
                        {
                          required: true,
                        ...
                        },
                        {
                          validator: (_, value) => {
                            ...
                            return Promise.reject(new Error('すでに登録された検査名称(略名)です'));
                          }
                        },
                      ]}
                    >
                      <Input style={{ border: 'none' }} maxLength={8}
                        onChange={(e) => { this.updateDatasource(index, 'exam_name_short_name', e.target.value) }}
                      />
                    </Form.Item>
                  )
                }}
                  ...
                  return (
                    ...
                    >
                      <Input style={{ border: 'none' }}
                        onChange={(e) => { this.updateDatasource(index, 'exam_code', e.target.value) }}
                      />
                    </Form.Item>
                  )
                }}
