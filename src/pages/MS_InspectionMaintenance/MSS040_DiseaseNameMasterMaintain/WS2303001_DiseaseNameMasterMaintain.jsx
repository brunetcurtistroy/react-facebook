...
class WS2303001_DiseaseNameMasterMaintain extends React.Component {
  ...
  loadMainSource = () => {
    ...
    axios.get('/api/disease-name-master-maintain/disease-name-master-maintain')
      .then(res => {
  ...
  onFinish = (values) => {
    this.setState({ isLoading: true });
    axios.post('/api/disease-name-master-maintain/disease-name-master-maintain', values)
      .then(res => {
      ...
      <div className="insure-guide-course">
        <Card title="病名マスタ保守">
          <Form ref={this.formRef} onFinish={this.onFinish}>
            ...
            >
              <Table.Column title="病名コード" dataIndex="disease_name_code" render={(value, record, index) => (<>
                <Form.Item name={['MainSource', index, 'id']} hidden>
                  <Input />
                </Form.Item>
                <Form.Item name={['MainSource', index, 'disease_name_code']}>
                  <Input />
                </Form.Item>
              </>)} />
              <Table.Column title="検索キー" dataIndex="search_key" render={(value, record, index) => (
                <Form.Item name={['MainSource', index, 'search_key']}>
                  <Input />
                </Form.Item>
              )} />
              <Table.Column title="病名" dataIndex="name_of_a_disease" render={(value, record, index) => (
                <Form.Item name={['MainSource', index, 'name_of_a_disease']}>
                  <Input />
                </Form.Item>
              )} />
              <Table.Column
                ...
                render={(value, record) => (<Space>
                  <Popconfirm title="削除しますか？" onConfirm={() => {
                    ...
                    return axios.post('/api/disease-name-master-maintain/disease-name-master-maintain/delete', {id: record.id})
