...
import { Button, Card, Checkbox, DatePicker, Form, Input, message, Modal, Select, Space, Spin } from "antd";
...
import DeleteAction from 'redux/InspectionMaintenance/InspectItemJudgeValueSetting/Delete.actions';
class WS0367003_Delete extends React.Component {
  static propTypes = {
  ...
  GetScreenData() {
    ...
    DeleteAction.GetScreenData(obj).then(res => {  
      this.setState({
      ...
      res.StartDateFChar = this.CheckDateNull(res.StartDateFChar) ? null : res.res.StartDateFChar
      res.GenderF = parseInt(res.GenderF) === 0 ? "" : parseInt(res.GenderF)
      res.JudgeLevelF = parseInt(res.JudgeLevelF) 
  ...
  Run_F12(){
    ...
    this.setState({loadding: true}) 
    DeleteAction.Run_F12(obj).then(res=>{ 
...
export default connect(mapStateToProps, mapDispatchToProps)(WS0367003_Delete);
